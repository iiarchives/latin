...
// Commands
if (isArgument(0, [null, "--help", "-h"]))
    printSuccess(await fileRead("texts/help.txt"));
else if (isArgument(0, ["--format", "-f"])) {
    let inputFile = getArgument(1);
    if (inputFile === null)
        printError("Input file is unspecified.");
    if (!await fileExists(inputFile))
        printError("Input file is not found.");
    let data = format(await fileRead(inputFile));
    let outputFile = getArgument(2);
    if (typeof outputFile === "string")
        await fileWrite(outputFile, data);
    else
        printSuccess(data);
}
